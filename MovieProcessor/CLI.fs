...
"""
    ...
    let stringOfMovieNames (movies : Movie seq) (separator : string) =
        let mapped = Seq.map (fun (m : Movie) -> $"{m.MovieId} - {m.Titles.GetEnumerator().Current}") movies
        String.Join(separator, mapped)

    let stringOfTags (tags: Tag seq) =
        String.Join(", ", tags)

    let printTags (tagsOrdered : KeyValuePair<string, Tag> array) (separator: string) (start : int)=
        let mapped = Seq.mapi (fun i -> fun (item : KeyValuePair<string, Tag>) -> $"{start + i} - {item.Key} ({item.Value.Movies.Count})") tagsOrdered
        String.Join(separator, mapped) |> printfn "%s"

    let printTag (identifier : string) (tagsOrdered : KeyValuePair<string, Tag> array) (tagsDict : IDictionary<_,Tag>) =
        ...
        match result with
        | Some tag -> printfn $"Movies for {tag.Key}: {stringOfMovieNames tag.Value.Movies comma}"
    ...
    let printMovie (identifier : string) (dataset : Dataset) =
        ...
        match result with
        | Some movie ->
            printfn $"Title - {movie.Titles.GetEnumerator().Current}"
            printfn $"Director - {stringOfPeople movie.Directors comma}"
            printfn $"Cast - [{stringOfPeople movie.Actors comma}]"
            printfn $"Tags - [{stringOfTags movie.Tags}]"
    ...
    let printTagPage (page : string) (tagsOrdered : KeyValuePair<string, Tag> array) =
    ...
    let rec run (data : Dataset) =
        ...
        let tagsOrdered = data.tags |> Seq.sortByDescending (_.Value.Movies.Count) |> Seq.toArray
        match input with
